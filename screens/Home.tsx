...
const Home: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setExpense(StorageService.getExpense());

    // Prepare simple chart data from transactions (last 7 items reversed for graph)
    const graphData = loadedTransactions.slice(0, 7).reverse().map((t, index) => ({
      name: index.toString(),
      value: Math.abs(t.amount) // Just visualization of magnitude
    }));
    
    // If empty, show flat line
        ...
        {transactions.length === 0 ? (
          ...
          <div className="space-y-3">
            {transactions.slice(0, 3).map((t) => (
              <div key={t.id} className="glass-panel p-3 rounded-2xl flex items-center justify-between border border-white/60">
                <div className="flex items-center gap-3">
                  <div className={`w-12 h-12 rounded-full ${t.iconColor} flex items-center justify-center`}>
